        ...
        receive
            {point, Point, _} ->
                case queue:len(Points) of
                    2 ->
                        liner_handler(queue:drop(queue:in(Point, Points)),
                                    Step,
                                    OutputPid);
                    1 ->
    ...
    NewPoints =
        receive
            {point, Point, _} ->
                case queue:len(Points) of
                    Window ->
                        lag_handler(queue:drop(queue:in(Point, Points)),
                                    Step,
                                    OutputPid);
                    WindowPrev ->
    ...
    NewPoints =
        receive
            {point, Point, _} ->
                case queue:len(Points) of
                    Window ->
                        new_handler(queue:drop(queue:in(Point, Points)),
                                    Window,
                                    Step,
                                    OutputPid);
                    WindowPrev ->
